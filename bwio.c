...
int bwputc( int channel, char c ) {
	...
	case COM1:
		...
		// wait until modem has CTS=1:
		modem = (int *)(UART1_BASE + UART_MDMSTS_OFFSET);
		while ( !(*UART1_MODEM_STATUS & MODEM_CTS_MASK) );
		break;
	case COM2:
...
int bwgetc( int channel ) {
	...
	case COM1:
		...
		modem = (int *)(UART1_BASE + UART_MDMCTL_OFFSET);
		while ( !(*modem & 2) ); // wait for RTS=1 in UART1ModemCtrl
